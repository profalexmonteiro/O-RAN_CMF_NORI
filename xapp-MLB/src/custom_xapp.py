...
# MLB decision table
#
# Mirrors, value for value, LoadToCio() of the nori-cmf.cc ns-3 example
# (contrib/nori/examples/nori-cmf.cc), which in turn reproduces the table
...
# built-in MLB emulation of nori-cmf.cc converge to the same decision for the
# same observed load, whichever one is actually driving a given cell.
# ---------------------------------------------------------------------------


def load_to_cio(load: float) -> float:
    """Cell load (0..1) -> Cell Individual Offset [dB].
    0.00% - 45.45% -> 0 dB, then +0.5 dB every 9.09% (1/11), up to 3.5 dB at 100%.
    """
    idx = min(max(int(math.floor(load * 11.0)), 0), 11)
    return 0.5 * max(0, idx - 4)


# RAN parameter IDs, matching the RicControlCallback() convention in nori-cmf.cc.
RAN_PARAM_CIO = 1  # HOMeasurementOffset, written by this xApp
RAN_PARAM_HYSTERESIS = 2  # HOHysteresis, written by the MRO xApp (read-only here)
RAN_PARAM_TTT = 3  # HOTimeToTrigger, written by the MRO xApp (read-only here)

# ricRequestorID used for control requests issued by this xApp. Distinct from
# the TS/QoS/RAN_SLICING IDs (1001/1002/1003) and from the MRO xApp's 1004;
# not otherwise interpreted by nori-cmf.cc, which decodes the control message
# content regardless of this value.
MLB_REQUESTOR_ID = 1005
...
class XappMlb:
    """
    MLB (Mobility Load Balancing) xApp for the NORI ns-3 module.

    Subscribes to the KPM RAN function (200) of every registered E2 node. On
    each RIC Indication, it reads the cell's PRB utilisation exposed by the
    nori-cmf.cc scenario, computes the same Cell Individual Offset (CIO)
    decision the scenario's built-in MLB emulation would compute, and-when it
    differs from what the cell currently has-writes it back as a RIC Control
    Request on the RAN Control function (300).

    This xApp only ever touches HOMeasurementOffset (CIO). It never writes
    HOHysteresis or HOTimeToTrigger: those parameters are the MRO xApp's
    responsibility. Because all three parameters live in the same
    "CellAffectHandoverBoundary" group, running this xApp alongside an MRO
    ...
    def __init__(self):
        ...
        self.logger = Logger(
            name="XappMlb", level=Level.DEBUG
        )  # The name is included in each log entry, Levels: DEBUG < INFO < WARNING < ERROR
        self.logger.info("Initializing the MLB xApp.")
    ...
    def subscribe_to_e2_nodes(self):
        ...
        sub_trs_id = self._rmrxapp.sdl_get(
            namespace="xappmlb", key="subscription_transaction_id"
        )
    ...
    def ric_indication_handler(self, rmrxapp: RMRXapp, summary: dict, sbuf):
        """
        Handler for RIC indication messages. Runs the MLB decision loop for the
        cell that sent this indication and, if the outcome differs from the
        cell's current CIO, replies with a RIC Control Request on the same
        route (rmr_rts), targeting that same cell.
        """
        ...
        # ---------------- MLB decision loop ----------------
        self.run_mlb_step(ric_indication_data, rmrxapp, sbuf)
    ...
    def run_mlb_step(self, ric_indication_data: dict, rmrxapp: RMRXapp, sbuf):
        """
        Reads the PRB utilisation and the currently active CIO reported by the
        cell, computes the MLB decision for this control period, and sends a
        RIC Control Request back to the same cell only when the decision
        differs from what is already active.
        """
        ...
        prb_used = float(pm.get("RRU.PrbUsedDl", 0) or 0)
        prb_avail = float(pm.get("RRU.PrbAvailDl", 0) or 0)
        load = min(1.0, prb_used / prb_avail) if prb_avail > 0 else 0.0

        cio_milli = pm.get("MLB.CioMilliDb")
        current_cio = cio_milli / 1000.0 if cio_milli is not None else None

        # Informational only: MRO owns these, this xApp never writes them.
        ...
        new_cio = load_to_cio(load)

        decisions: List[Tuple[int, float]] = []
        if current_cio is None or abs(new_cio - current_cio) > DECISION_EPSILON:
            decisions.append((RAN_PARAM_CIO, new_cio))

        if self.save_influx:
            try:
                self.send_mlb_influxdb_point(
                    cell_object_id,
                    prb_used,
                    prb_avail,
                    load,
                ...
                self.logger.warning(f"Failed to write MLB point to InfluxDB: {exc}")
...
>>>>>>> 8d6507985e9043f9dd0d376a634f6354e7703e38
    def build_ric_control_pdu(self, decisions: List[Tuple[int, float]]) -> bytes:
        ...
        in milli-units (milli-dB for CIO), matching the convention decoded by
                        ...
                        "protocolIEs": [
                            {
                                ...
                                "value": (
                                    "RICrequestID",
                                    {
                                        "ricRequestorID": MLB_REQUESTOR_ID,
                                        "ricInstanceID": 1,
                                    },
                                ),
                            },
    ...
    def send_mlb_influxdb_point(
        self,
        cell_object_id: str,
        prb_used: float,
        prb_avail: float,
        load: float,
        current_cio: Optional[float],
        new_cio: float,
        current_hyst: Optional[float],
        current_ttt: Optional[float],
    ):
        """
        Records one MLB decision-loop sample per cell per indication, so the
        ...
        p = (
            Point("mlb_control")
            .tag("cellObjectID", cell_object_id)
            .field("prb_used", prb_used)
            .field("prb_avail", prb_avail)
            .field("load", load)
            .field("new_cio_db", new_cio)
        )
        if current_cio is not None:
            p = p.field("current_cio_db", current_cio)
    ...
    def generate_subscription_request(
        ...
        return {
            "SubscriptionId": "",
            "ClientEndpoint": {
                "Host": "service-ricxapp-xappmlb-http.ricxapp",
                "HTTPPort": 8080,
                "RMRPort": 4560,
            },
